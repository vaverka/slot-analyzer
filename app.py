# ==============================================================================
#  app.py - УНИВЕРСАЛЬНЫЙ АНАЛИЗАТОР СЛОТОВ V7.9 (фикс шага ставки по умолчанию = min_bet)
# ==============================================================================
...
class SlotProbabilityCalculator:
    def __init__(self, config):
        ...
        # читаем параметры ставок один раз
        ...
        # как float для формул
        self.min_bet = float(bet_range[0])
        self.max_bet = float(bet_range[1])

        # ВАЖНО: шаг ставки. Если нет явного ключа — используем min_bet (как в твоей версии).
        self.bet_step = float(
            gc.get('bet_step') or
    ...
    # --- Вспомогательное округление к шагу ---
    def _floor_to_step(self, value, step):
        """Округление вниз к ближайшему кратному шагу с Decimal для избежания float-хвостов."""
        v = Decimal(str(value))
        s = Decimal(str(step))
        if s <= 0:
            return float(v)
    ...
    def visualize_win_probabilities(self, level='base'):
        ...
        ax.set_xlabel('Вероятность за один спин (с учетом Wild), %', fontsize=12)
    ...
    if analyze_button and config_file is not None:
        try:
            ...
            with st.expander("Как понимать эти цифры? 🤔"):
                ...
                - **Как определяется ставка?** Мы умножаем минимальную ставку слота (**${calculator.min_bet:.2f}**) на коэффициент риска (x1-x5) и на нелинейный коэффициент вашего банкролла. Затем результат **округляется по реальному шагу ставки (${calculator.bet_step:.2f})** и корректируется под лимиты слота.
            ...
            with st.container(border=True):
                ...
                st.success(f"**Результат**: Итоговый рекомендуемый минимум составляет **${min_bankroll_value:,.2f}**")

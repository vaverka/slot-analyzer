# ==============================================================================
#  app.py - УНИВЕРСАЛЬНЫЙ АНАЛИЗАТОР СЛОТОВ V5.1 (финальная, практичная версия)
# ==============================================================================
import json
import math
import pandas as pd
...
def main():
    ...
    if analyze_button and uploaded_file:
        try:
            ...
            with st.expander("Как понимать эти цифры и что такое 'Гарантированное количество спинов'? 🤔"):
                st.markdown("""
                ...
                Ваш желаемый выигрыш — это не математически ожидаемый результат, а **счастливая случайность (положительное отклонение от среднего)**. Вы не "движетесь" к выигрышу с каждым спином. Вместо этого, вы медленно "дрейфуете" в сторону проигрыша, надеясь поймать одну или несколько крупных выигрышных комбинаций.

                #### 3. Гарантированные спины vs. Ожидаемое время игры
                - **Гарантированное количество спинов** (то, что мы показываем) — это `Ваш Банкролл / Ваша Ставка`. Это **практический минимум**, который вы можете сделать, если вам совсем не будет везти. Это ваш **реальный запас прочности**.
                - *Теоретическое ожидаемое время игры* (`Банкролл / (Ставка * (1 - RTP))`) всегда больше, так как учитывает мелкие выигрыши, которые возвращаются вам по RTP и немного продлевают сессию.
                
                **Для планирования всегда ориентируйтесь на Гарантированное количество спинов.** Это честная оценка вашего игрового потенциала.
                """)

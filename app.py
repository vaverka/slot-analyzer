# ==============================================================================
#  app.py - УНИВЕРСАЛЬНЫЙ АНАЛИЗАТОР СЛОТОВ V5.2 (финальная, логичная версия)
# ==============================================================================
import json
import math
import pandas as pd
...
# --- Класс-калькулятор (без изменений) ---
...
class SlotProbabilityCalculator:
    ...
    def generate_bankroll_strategy(self, personal_bankroll, risk_level='medium'):
        ...
        # <-- ИЗМЕНЕНИЕ: Расчет "Оптимальной ставки" на основе характеристик слота -->
        optimal_bet = min_bankroll * (profile['bet_percent'] / 100)
        # "Зажимаем" ставку между минимальной и максимальной
...
def main():
    ...
    if analyze_button and uploaded_file:
        try:
            ...
            # <-- ИЗМЕНЕНИЕ: Новый, практичный и понятный расчет спинов -->
            optimal_bet = strategy.get('optimal_bet')
            spins_at_optimal_bet = int(personal_bankroll / optimal_bet) if optimal_bet > 0 else float('inf')
            ...
            with col2:
                # <-- ИЗМЕНЕНИЕ: Выводим новую, понятную метрику -->
                spins_str = f"{spins_at_optimal_bet}" if spins_at_optimal_bet != float('inf') else "∞"
                st.metric(label=f"Спинов при Оптимальной Ставке (${optimal_bet:.2f})", value=spins_str)
            ...
            with st.expander("Как понимать эти цифры? 🤔"):
                ...
                - **Почему эта ставка "Оптимальная"?** Она основана не на вашем личном банке, а на **рекомендуемом минимальном банкролле** для этого слота. Это сбалансированная ставка, позволяющая достаточно долго оставаться в игре, чтобы дать шанс крупным выигрышам.
                - **Ваш банкролл определяет "запас прочности"**: Чем больше ваш банк, тем больше "Оптимальных Ставок" вы можете сделать, и тем дольше ваше игровое время для достижения цели.
                """)

            st.subheader("📊 Визуальный анализ вероятностей", divider="blue")
            # ... (вывод графиков и таблиц без изменений) ...

            st.header("♟️ Персональная стратегия игры", divider="rainbow")
            # ... (вывод стратегии без изменений) ...

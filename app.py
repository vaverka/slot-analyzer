    ...
    if analyze_button and uploaded_file:
        try:
            ...
            with st.expander("Как понимать эти цифры и почему мы считаем спины 'до разорения'? 🤔"):
                st.markdown("""
                Это ключевой момент для понимания стратегии игры в слоты... (Ваше развернутое объяснение, как в предыдущей версии)
                """)
            ...
            with st.container(border=True):
                ...
                # <-- ИСПРАВЛЕНИЕ: Используем st.markdown для правильного форматирования -->
